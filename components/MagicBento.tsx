...
/* ─── Reusable grid props ─── */
export interface MagicBentoGridProps extends BentoProps {
  children: React.ReactNode;
  className?: string;
  /** Extra CSS grid class to override the default responsive layout */
...
const GlobalSpotlight: React.FC<{
  ...
  useEffect(() => {
    ...
    const handleMouseLeave = () => {
      gridRef.current?.querySelectorAll('.magic-card').forEach(card => { (card as HTMLElement).style.setProperty('--glow-intensity', '0'); });
...
/* ═══════════════════════════════════════════════════════════════
   EXPORTED: MagicBentoCard — a single interactive card wrapper
   ═══════════════════════════════════════════════════════════════ */
export const MagicBentoCard: React.FC<MagicBentoCardProps> = ({
  ...
  const cardClassName = `magic-card flex flex-col justify-between relative w-full p-5 rounded-[20px] border border-solid font-light overflow-hidden transition-colors duration-300 ease-in-out hover:-translate-y-0.5 hover:shadow-[0_8px_25px_rgba(0,0,0,0.15)] ${enableBorderGlow ? 'card--border-glow' : ''} ${className}`;

  const cardStyle: React.CSSProperties = {
    backgroundColor: '#0a0a0a',
    borderColor: 'rgba(215,190,89,0.15)',
    color: '#eeeeee',
    '--glow-x': '50%',
    '--glow-y': '50%',
    '--glow-intensity': '0',
...
/* ═══════════════════════════════════════════════════════════════
   EXPORTED: MagicBentoGrid — wraps children with spotlight + glow CSS
   ═══════════════════════════════════════════════════════════════ */
export const MagicBentoGrid: React.FC<MagicBentoGridProps> = ({
  ...
  return (
    <>
      ...
      <div
        className={`bento-section select-none relative ${gridClassName || ''} ${className}`}
        ref={gridRef}
      >
        {children}
      </div>
...
/* ═══════════════════════════════════════════════════════════════
   DEFAULT EXPORT: Original self-contained MagicBento demo
   ═══════════════════════════════════════════════════════════════ */
      ...
      >
        {cardData.map((card, index) => (
          <MagicBentoCard
            ...
            style={{ backgroundColor: card.color || '#120F17', borderColor: '#2F293A' }}
          >
