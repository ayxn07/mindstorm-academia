...
export default function FAQHero() {
  return (
    <section className="relative h-screen flex items-center justify-center overflow-hidden">
      ...
      {/* Floating particles */}
      <motion.div
        className="absolute top-[20%] left-[10%] w-1 h-1 rounded-full opacity-40"
        style={{ backgroundColor: "#d7be59" }}
        animate={{ y: [-10, 10, -10], opacity: [0.3, 0.7, 0.3] }}
        transition={{ duration: 4, repeat: Infinity, ease: "easeInOut" }}
      />
      <motion.div
        className="absolute top-[60%] right-[15%] w-1.5 h-1.5 rounded-full opacity-30"
        style={{ backgroundColor: "#d7be59" }}
        animate={{ y: [10, -15, 10], opacity: [0.2, 0.6, 0.2] }}
        transition={{ duration: 5, repeat: Infinity, ease: "easeInOut", delay: 1 }}
      />
      <motion.div
        className="absolute top-[40%] left-[80%] w-1 h-1 rounded-full opacity-25"
        style={{ backgroundColor: "#d7be59" }}
      ...
      >
        ...
        >
          Get answers to common questions about our services, the application
          process, and how we help students achieve their international education
          goals.
        </motion.p>
      </motion.div>
    </section>
